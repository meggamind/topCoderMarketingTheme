<?php
/**
 * Topcoder 2 Marketing Theme Primary Menu Walker
 *
 * @package Topcoder_2_Marketing
 */
 class TC_Primary_Menu_Walker extends Walker_Nav_Menu {
   private $curItem;
  ...
  */
  function start_lvl( &$output, $depth = 0, $args = array() ) {
     ...
     $classes = empty( $this->curItem->classes ) ? array() : (array) $this->curItem->classes;
     $classes_to_apply = array(
       'bar-submenu'
     );
     if( $this->is_active($classes) ){
       $classes_to_apply[] = 'active';
     }
     $class_names = implode( ' ', $classes_to_apply );

     // Build HTML for output.
     $output .= "\n" . $indent . '<nav id="submenu-'. $this->curItem->ID .'" class="' . $class_names . '"><div class="submenu-con flex middle"><ul class="submenu-list flex">' . "\n";
  }
  ...
  function end_lvl( &$output, $depth = 0, $args = array() ) {
    ...
    // Build HTML for output.
    $output .= "\n" . $indent . '</ul></div><div class="submenu-toggle-icon"></div></nav>' . "\n";
  }
